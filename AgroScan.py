...
from typing import Optional  # FIX: fallback para Optional quando necessário
...
def inject_global_css():
    ...
    palette = {
        ...
        "Claro": {
            ...
            "topbar_bg": "rgba(247, 251, 248, 0.97)",
        },
    }
...
.ag-topbar-tab.active {{
  color: var(--ag-success) !important;
  border-bottom-color: var(--ag-success);
...
section[data-testid='stTabs'], div[data-testid='stTabs'] {{
  position: sticky;
  top: 56px;
  z-index: 10010;
...
def render_topbar(active_tab: str) -> str:
    """
    Renderiza a barra fixa no topo com as abas 'Registro de Propriedade' e 'Análise Geral'.
    Retorna o nome da aba ativa (persiste via query_params / session_state).
    """
    tabs = [
        ("🏡 Registro de Propriedade", "registro"),
        ("📊 Análise Geral",           "analise"),
    ]
    itens_html = ""
    for label, key in tabs:
        css_class = "ag-topbar-tab active" if key == active_tab else "ag-topbar-tab"
        itens_html += (
            f"<a class='{css_class}' href='?tab={key}' target='_self'>"
            f"{label}</a>"
        )
    ...
    st.markdown(
        f"""
        <div class='ag-topbar'>
            <span class='ag-topbar-brand'>🌱 AgroScan</span>
            {itens_html}
        </div>
        <div class='ag-content-offset'></div>
        """,
        unsafe_allow_html=True,
    )
    return active_tab
...
def get_active_tab() -> str:
    """Lê a aba ativa via query params."""
    params = st.query_params
    tab = params.get("tab", "analise")
    if tab not in ("registro", "analise"):
        tab = "analise"
    return tab
...
def typewriter(texto: str, tag: str = "h3", extra_style: str = ""):
    st.markdown(
...
def fade_slide_wrap(html_items: list) -> str:
...
def render_priority_notice(title: str, message: str, level: str = "info"):
    css_class = {"high": "high", "medium": "medium", "low": "low", "info": ""}.get(level, "")
...
def render_interactive_geo_map(
    ...
    # FIX: usa Optional[float] em vez de float | None para compatibilidade com Python 3.8/3.9
...
def redimensionar(img: np.ndarray, max_w: int = MAX_W, max_h: int = MAX_H) -> np.ndarray:
    h, w = img.shape[:2]
...
def img_to_base64(img: np.ndarray, quality: int = 85) -> str:
    _, buf = cv2.imencode(".jpg", img, [cv2.IMWRITE_JPEG_QUALITY, quality])
    return base64.b64encode(buf).decode()


def img_rgb_to_base64(img_rgb: np.ndarray, quality: int = 85) -> str:
...
def carregar_imagem_bgr(path: Optional[str]) -> Optional[np.ndarray]:
    # FIX: usa Optional em vez de str | None
...
def ensure_optional_dependency(module_name: str, pip_name: Optional[str] = None):
    # FIX: retorno sem anotação de tuple para evitar erro de sintaxe em Python 3.8
...
def formatar_numero_br(valor: Optional[float], casas: int = 2) -> str:
    if valor is None:
        return "N/D"
...
def obter_indicadores_municipio_ibge(codigo_municipio: Optional[int]) -> dict:
    ...
    try:
        ...
        if not isinstance(dados, list) or len(dados) <= 1:
            return {}
        indicadores = {"ano_base": "2022", "fonte_indicadores": "IBGE SIDRA / Censo 2022"}
        for item in dados[1:]:
...
def salvar_propriedade(user_id: int, nome: str, cidade: str,
    ...
    conn.execute(
        ...
        """,
        (
            user_id, nome, cidade.strip(),
            float(latitude), float(longitude),
            float(area_ha), observacoes.strip(),
...
def listar_propriedades(user_id: int) -> list:
    conn = get_db_connection()
    rows = conn.execute(
...
def obter_previsao_tempo(latitude: float, longitude: float) -> dict:
    try:
        ...
        for data, tmax, tmin, chuva in zip(datas, maxs, mins, chuvas):
            previsao.append({"data": data, "temp_max": tmax, "temp_min": tmin, "chuva_mm": chuva})
        ...
        for data, temp, umid, prob, chuva in zip(horas, temps, umidades, probs, chuvas_h):
            previsao_24h.append({"data": data, "temperatura": temp, "umidade": umid, "prob_chuva": prob, "chuva_mm": chuva})

        chuva_24h = round(sum(float(item.get("chuva_mm") or 0) for item in previsao_24h), 1)
        pico_temp_24h = round(max(
            (float(item.get("temperatura") or 0) for item in previsao_24h),
            default=float(atual.get("temperature_2m") or 0)
        ), 1)
        ...
        return {
            ...
            "janela_24h": {"chuva_total_mm": chuva_24h, "pico_temp": pico_temp_24h, "umidade_media": umidade_media_24h},
            "janela_3d": {"chuva_total_mm": chuva_3d, "dias_monitorados": len(previsao[:3])},
            "janela_7d": {"chuva_total_mm": chuva_7d, "dias_monitorados": len(previsao[:7])},
...
def prever_tipo_modelo_local(ndvi: float, textura: float, umidade_frac: float, sat_mean: float):
    conn = get_db_connection()
    rows = conn.execute(
        """
        SELECT label, COUNT(*) AS total,
               AVG(ndvi) AS ndvi, AVG(textura) AS textura,
               AVG(umidade) AS umidade, AVG(saturacao) AS saturacao
        FROM model_samples
        GROUP BY label
        HAVING COUNT(*) >= 3
        """
    ...
    for row in rows:
        ...
        centro = np.array([
            float(row["ndvi"]), float(row["textura"]) / 80.0,
            float(row["umidade"]), float(row["saturacao"]),
        ], dtype=float)
...
def registrar_amostras_modelo(user_id: int, property_id: int, analises: dict):
    ...
    for dados in analises.values():
        conn.execute(
            ...
            """,
            (
                user_id, property_id, dados["tipo"],
                float(dados["ndvi"]), float(dados["textura"]),
                float(dados["umidade_frac"]), float(dados.get("sat_mean", 0.0)),
                agora,
            ),
        )
    conn.commit()
...
def registrar_feedback_usuario(
    user_id: int, property_id: Optional[int], analysis_id: Optional[int],
    grid_name: str, dados_grid: dict, rotulo_final: str,
    risco_confirmado: str, observacao: str = ""
):
    ...
    try:
        conn.execute(
            ...
            (
                user_id, property_id, analysis_id, grid_name,
                dados_grid.get("tipo"), rotulo_final, risco_confirmado,
                observacao.strip(), agora,
            ),
        )
        conn.execute(
            """
            ...
            """,
            (
                user_id, property_id, rotulo_final,
                float(dados_grid.get("ndvi", 0.0)), float(dados_grid.get("textura", 0.0)),
                float(dados_grid.get("umidade_frac", 0.0)), float(dados_grid.get("sat_mean", 0.0)),
                agora,
            ),
        )
        conn.commit()
...
def detectar_riscos(analises: dict, metricas: dict, clima: Optional[dict] = None) -> list:
    riscos = []
    ...
    def add_risk(titulo: str, nivel: str, descricao: str, acao: str):
        riscos.append({"titulo": titulo, "nivel": nivel, "descricao": descricao, "acao": acao})
...
def gerar_resumo_executivo(resultado_geral: str, metricas: dict, riscos: list) -> list:
    resumo = [
...
def descrever_localizacao_grid(grid_idx: int, rows: int, cols: int, area_total_ha: float = 0.0) -> dict:
    ...
    return {"local": local, "vertical": vertical, "horizontal": horizontal, "area_pct": area_pct, "area_ha": area_ha}


def identificar_riscos_localizados(
    analises: dict, coords: dict, clima: Optional[dict],
    rows: int, cols: int, area_total_ha: float = 0.0
) -> list:
    pontos = []
    ...
    def adicionar(nome_grid, idx, titulo, nivel, descricao, acao, gatilho_clima, score_ref):
        xs, xe, ys, ye = coords[nome_grid]
        posicao = descrever_localizacao_grid(idx, rows, cols, area_total_ha)
        pontos.append({
            "grid": nome_grid, "titulo": titulo, "nivel": nivel,
            "localizacao": posicao["local"], "area_pct": posicao["area_pct"],
            "area_ha": posicao["area_ha"], "descricao": descricao,
            "acao": acao, "gatilho_clima": gatilho_clima,
    ...
    for idx, (nome, dados) in enumerate(analises.items()):
        ...
        if tipo == "Seco" or ndvi < 0.10 or (temp_atual >= 32 and chuva_prevista < 6 and umidade_frac < 0.18):
            ...
            adicionar(nome, idx, "Seca localizada", nivel,
                      ...
                      f"Janela climática: {chuva_prevista:.1f} mm previstos e temperatura atual de {temp_atual:.1f} °C.", score)
        ...
        if tipo == "Umido" or umidade_frac > 0.38 or (chuva_prevista > 20 and umidade_frac > 0.20):
            ...
            adicionar(nome, idx, "Encharcamento provável", nivel,
        ...
        if tipo == "Rochoso" or textura > 42 or (tipo == "Arenoso" and chuva_prevista > 18):
            ...
            adicionar(nome, idx, "Erosão / solo exposto", nivel,
                      ...
                      f"Risco ampliado com chuva prevista de {chuva_prevista:.1f} mm sobre solo mais exposto.", score)

        if umidade_ar > 80 and 0.10 < ndvi < 0.32:
            adicionar(nome, idx, "Pragas e fungos", "Médio",
                      ...
                      f"Umidade relativa do ar em {umidade_ar:.0f}% no momento.", score)

    pontos.sort(key=lambda item: (prioridade.get(item["nivel"], 0), 100 - item["score_referencia"]), reverse=True)
    return pontos[:16]


def gerar_mapa_riscos_localizados(img: np.ndarray, riscos_localizados: list) -> np.ndarray:
    mapa = img.copy()
    if not riscos_localizados:
        return mapa
    severidade = {"Baixo": 1, "Médio": 2, "Alto": 3}
    riscos_dominantes: dict = {}
...
def gerar_alertas_inteligentes(
    metricas: dict, riscos: list, clima: Optional[dict], riscos_localizados: list
) -> list:
    alertas = []
    prioridade = {"Baixo": 1, "Médio": 2, "Alto": 3}
    ...
    def adicionar(nivel, titulo, janela, descricao, acao):
        alertas.append({"nivel": nivel, "titulo": titulo, "janela": janela, "descricao": descricao, "acao": acao})

    if any(r.get("titulo") == "Seca localizada" and r.get("nivel") == "Alto" for r in riscos_localizados) and chuva_3d < 8:
        adicionar("Alto", "Irrigação prioritária", "Próximas 24h",
    ...
    if any(r.get("titulo") == "Encharcamento provável" for r in riscos_localizados) and chuva_3d >= 20:
        adicionar("Alto" if chuva_3d >= 35 else "Médio", "Drenagem preventiva", "Próximos 3 dias",
    ...
    if umidade_ar >= 80 and any("Pragas e fungos" in r.get("titulo", "") for r in riscos_localizados + riscos):
        adicionar("Médio", "Janela fitossanitária crítica", "48h",
    ...
    if vento >= 18:
        adicionar("Médio", "Operação de campo com restrição", "Hoje",
    ...
    if ndvi_global >= 0.22 and chuva_7d <= 12 and not alertas:
        adicionar("Baixo", "Monitoramento programado", "Próximos 7 dias",
...
def montar_plano_acao_prioritario(
    analises: dict, riscos_localizados: list,
    clima: Optional[dict], propriedade: Optional[dict] = None
) -> list:
    plano = []
    vistos = set()
    ...
    for item in riscos_localizados:
        ...
        plano.append({
            "prioridade": len(plano) + 1, "objetivo": item.get("titulo", "Área de atenção"),
            "grid": grid, "setor": item.get("localizacao", "Setor não identificado"),
            "acao": item.get("acao", "Monitorar em campo."), "prazo": prazo,
    ...
    if not plano and analises:
        ...
        for nome, dados in ordenados:
            plano.append({
                "prioridade": len(plano) + 1, "objetivo": f"Melhorar score agrícola de {nome}",
                "grid": nome, "setor": "Setor monitorado",
...
def estimar_coordenadas_talhoes_risco(riscos_localizados: list, propriedade: Optional[dict]) -> list:
...
def salvar_analise_historico(
    user_id: int, property_id: int, origem_imagem: str,
    img_orig: np.ndarray, img_mapa: np.ndarray,
    resultado_geral: str, metricas: dict, analises: dict,
    clima: dict, riscos: list,
    coords: Optional[dict] = None,
    riscos_localizados: Optional[list] = None
) -> int:
    ensure_data_dirs()
    ...
    cur.execute(
        ...
        """,
        (
            user_id, property_id, origem_imagem, image_path, map_path,
            resultado_geral,
...
def listar_analises_salvas(user_id: int, property_id: Optional[int] = None) -> list:
    conn = get_db_connection()
    query = (
...
# ═══════════════════════════════════════════════════════════════
# MAPA DE TERRENO — TURBO colormap
# ═══════════════════════════════════════════════════════════════
...
def gerar_mapa_terreno(img: np.ndarray) -> np.ndarray:
    ...
    mapa = cv2.applyColorMap(ndvi_norm, cv2.COLORMAP_TURBO)
    return mapa
...
def analisar_regiao(img_area: np.ndarray) -> dict:
    ...
    tipo_modelo, confianca_modelo, total_amostras = prever_tipo_modelo_local(ndvi, textura, umidade_frac, sat_mean)
    ...
    irrig_map = {"Fertil": 4.0, "Umido": 1.5, "Seco": 8.0, "Arenoso": 6.0, "Rochoso": 0.0}

    return {
        "tipo": tipo, "tipo_heuristico": tipo_heuristico,
        "tipo_modelo": tipo_modelo or "Indisponível",
        "origem_modelo": origem_modelo, "confianca_modelo": confianca_modelo,
        "ndvi": round(ndvi, 3), "textura": round(textura, 1),
        "umidade_frac": round(umidade_frac, 3), "sat_mean": round(sat_mean, 3),
        "score": score, "culturas": culturas_map.get(tipo, ["Indefinido"]),
...
def recomendacao_local(dados: dict) -> dict:
    ...
    rec = {
        "Fertil": {
            "opcao_1": {"perfil": "Producao convencional", "culturas": "Soja, Milho ou Feijao",
                        "plantio": f"Solo em boas condicoes (Score {score}/100). Plantio direto recomendado.",
                        "irrigacao": f"~{irr} mm/dia via aspersao convencional.",
                        "acao": "Analise de solo a cada 2 anos. Rotacao com gramíneas."},
            "opcao_2": {"perfil": "Agricultura organica", "culturas": "Hortalicas, Frutas ou Leguminosas",
                        "plantio": f"NDVI {ndvi} indica boa biomassa. Ideal para transicao organica.",
                        "irrigacao": f"~{irr-1:.1f} mm/dia via gotejamento subsuperficial.",
                        "acao": "Adubacao verde com crotalaria. Compostagem no proprio terreno."},
            "opcao_3": {"perfil": "Alta produtividade", "culturas": "Cana-de-acucar ou Eucalipto",
                        "plantio": f"Terreno com Score {score}/100 suporta culturas de ciclo longo.",
                        "irrigacao": f"~{irr+1:.1f} mm/dia com pivo central.",
                        "acao": "Calagem preventiva. Monitoramento mensal de pragas."},
        },
        "Umido": {
            "opcao_1": {"perfil": "Aproveitamento da umidade", "culturas": "Arroz irrigado ou Taro",
                        ...
                        "acao": "Nivelar o terreno para uniformizar a lamina."},
            "opcao_2": {"perfil": "Fruticultura adaptada", "culturas": "Banana, Coco ou Acai",
                        "plantio": f"NDVI {ndvi} indica condicoes favoraveis para fruticultura tropical.",
                        "irrigacao": f"~{irr-0.5:.1f} mm/dia. Drenagem lateral essencial.",
                        "acao": "Instalar valetas de drenagem. Cobertura de palha entre plantas."},
            "opcao_3": {"perfil": "Reflorestamento produtivo", "culturas": "Seringueira ou Palmito",
                        "plantio": f"Solo umido com Score {score}/100 e ideal para especies de alto valor.",
        ...
        "Seco": {
            "opcao_1": {"perfil": "Culturas resistentes a seca", "culturas": "Sorgo ou Mandioca",
                        "plantio": f"NDVI baixo ({ndvi}) confirma stress hidrico. Culturas CAM e C4 sao ideais.",
                        "irrigacao": f"~{irr} mm/dia gotejamento subsuperficial.",
                        "acao": "Mulching com palha seca para reduzir evaporacao."},
            "opcao_2": {"perfil": "Pecuaria extensiva", "culturas": "Palma forrageira ou Capim buffel",
                        "plantio": f"Score {score}/100. Forrageiras resistentes a seca.",
                        "irrigacao": f"~{irr-2:.1f} mm/dia. Cisternas de captacao como suporte.",
                        "acao": "Terraceamento para captacao de chuva."},
            "opcao_3": {"perfil": "Recuperacao gradual", "culturas": "Sisal ou Agave",
                        "plantio": f"Terreno seco com Score {score}/100 — plantas xerofitas preparam o solo.",
                        "irrigacao": f"~{max(1.0, irr-3):.1f} mm/dia apenas no estabelecimento.",
                        "acao": "Introducao de materia organica por compostagem."},
        },
        "Arenoso": {
            "opcao_1": {"perfil": "Hortalicas de raiz", "culturas": "Cenoura, Batata-doce ou Amendoim",
                        "plantio": f"Solo arenoso (textura {tex}) facilita crescimento de raizes. Score {score}/100.",
                        "irrigacao": f"~{irr} mm/dia fracionado em 3x ao dia.",
                        "acao": "Incorporar 20% de argila e composto organico antes do plantio."},
            "opcao_2": {"perfil": "Fruticultura irrigada", "culturas": "Melancia, Melao ou Morango",
                        "plantio": f"NDVI {ndvi} em solo arenoso indica boa drenagem para fruticultura.",
                        "irrigacao": f"~{irr-1:.1f} mm/dia via gotejamento com fertirrigacao.",
                        "acao": "Adubacao solavel parcelada semanalmente."},
            "opcao_3": {"perfil": "Melhoria estrutural do solo", "culturas": "Feijao-de-porco ou Mucuna",
                        "plantio": f"Score {score}/100. Ciclo de adubacao verde por 1-2 safras.",
                        "irrigacao": f"~{irr-2:.1f} mm/dia. Priorizar agua no florescimento.",
                        "acao": "Incorporar biomassa no solo apos florescimento."},
        },
        "Rochoso": {
            "opcao_1": {"perfil": "Uso minimo viavel", "culturas": "Cactos ornamentais ou Suculentas",
                        ...
                        "acao": "Subsolagem profunda para quebrar camada rochosa superficial."},
            "opcao_2": {"perfil": "Reflorestamento nativo", "culturas": "Especies nativas (aroeira, umbu)",
                        "plantio": f"NDVI {ndvi} em area rochosa — vegetacao nativa e a opcao mais sustentavel.",
                        "irrigacao": "Zero apos estabelecimento (90 dias).",
                        "acao": "Coveamento manual de 40x40x40 cm com substrato enriquecido."},
            "opcao_3": {"perfil": "Aproveitamento alternativo", "culturas": "Geracao fotovoltaica ou turismo rural",
                        "plantio": f"Score {score}/100 indica baixo potencial agricola. Uso alternativo e mais rentavel.",
                        "irrigacao": "Nao aplicavel.",
                        "acao": "Estudo topografico para implantacao de paineis solares ou trilhas."},
        },
    }
...
def analisar_com_ia(img: np.ndarray, analises: dict, api_key: str) -> str:
    try:
        ...
        for nome, d in analises.items():
            linhas.append(
                f"- {nome}: {d['tipo']} | NDVI={d['ndvi']} | Score={d['score']}/100 | Irrigacao={d['irrigacao_mm']}mm/dia"
            )
        resumo = "\n".join(linhas)
        ...
        msg = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": [
                    {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg", "data": img_to_base64(img, quality=75)}},
                    {"type": "text", "text": (
                        ...
                        "2. Para cada tipo de solo: culturas rentaveis, manejo de irrigacao, correcao de solo.\n"
...
def drone_animation_component(img: np.ndarray, analises: dict, coords: dict, rows: int, cols: int) -> str:
    h, w = img.shape[:2]
    ...
    for nome, dados in analises.items():
        ...
        grids.append({"nome": nome, "tipo": dados["tipo"], "score": dados["score"],
                      "xs": int(xs), "xe": int(xe), "ys": int(ys), "ye": int(ye)})
...
<div id="dl" style="font-family:monospace;font-size:12px;color:#aaa;min-height:20px;margin-top:5px;"></div>
</div>
<script>
(function(){{
  ...
  function grid(hl){{
    ...
    for(let j=1;j<COLS;j++){{const x=j*Math.floor(IW/COLS);ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,IH);ctx.stroke();}}
    for(let i=1;i<ROWS;i++){{const y=i*Math.floor(IH/ROWS);ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(IW,y);ctx.stroke();}}
    if(hl){{ctx.strokeStyle='rgba(255,210,0,0.95)';ctx.lineWidth=2.5;ctx.strokeRect(hl.xs,hl.ys,hl.xe-hl.xs,hl.ye-hl.ys);}}
  }}
  function drone(cx,cy,t){{
    const r=(t*60)%42;
    ctx.save();ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);
    ctx.strokeStyle=`rgba(0,255,100,${{Math.max(0,.65-r/55)}})`;ctx.lineWidth=1.2;ctx.stroke();ctx.restore();
    ctx.beginPath();ctx.arc(cx,cy,13,0,Math.PI*2);ctx.fillStyle='rgba(255,120,0,.95)';ctx.fill();
    ctx.beginPath();ctx.arc(cx,cy,5,0,Math.PI*2);ctx.fillStyle='rgba(255,240,0,.95)';ctx.fill();
  ...
  async function scan(g){{
    ...
    for(let yi=0;yi<5;yi++){{
      ...
      for(let xi=0;xi<18;xi++){{
        const f=xi/17;const px=yi%2===0?xs+(xe-xs)*f:xe-(xe-xs)*f;
        dx=(1-a)*dx+a*px;dy=(1-a)*dy+a*py;
        ctx.drawImage(bg,0,0,IW,IH);grid(g);drone(dx,dy,t);t+=0.016;await sleep(13);
      }}
    }}
...
def interactive_risk_map_component(img: np.ndarray, riscos_localizados: list, rows: int, cols: int) -> str:
    h, w = img.shape[:2]
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img_b64 = img_rgb_to_base64(img_rgb)
    prioridade = {"Baixo": 1, "Médio": 2, "Alto": 3}
    ...
    for risco in riscos_localizados:
        ...
        item = agrupados.setdefault(grid, {
            "grid": grid, "localizacao": risco.get("localizacao", "Faixa não identificada"),
            "area_pct": risco.get("area_pct"), "area_ha": risco.get("area_ha"),
            "bbox": risco.get("bbox", {}), "nivel": risco.get("nivel", "Médio"),
            "titulo": risco.get("titulo", "Área de atenção"),
            "gatilho_clima": risco.get("gatilho_clima", ""), "acao": risco.get("acao", ""), "detalhes": [],
        })
        item["detalhes"].append({
            "titulo": risco.get("titulo", "Risco"), "nivel": risco.get("nivel", "Médio"),
            "descricao": risco.get("descricao", ""), "acao": risco.get("acao", ""),
...
(function(){{
  ...
  function colorFor(level) {{ return colors[level] || '#37b5ff'; }}
  ...
  function renderDetail(idx) {{
    if (idx < 0 || !PINS[idx]) {{ detail.innerHTML = 'Selecione um ponto no mapa para abrir o detalhamento interativo.'; return; }}
    const pin = PINS[idx];
    ...
    detail.innerHTML = `<strong>${{pin.grid}}</strong> — ${{pin.localizacao}}<br/><span style="color:#b9dcb9">Área estimada:</span> ${{formatArea(pin)}} · <span style="color:#b9dcb9">Risco dominante:</span> ${{pin.titulo}} (${{pin.nivel}})<br/><span style="color:#b9dcb9">Gatilho climático:</span> ${{pin.gatilho_clima || 'Leitura local do terreno'}}<ul style="margin:8px 0 0 18px;padding:0">${{lista}}</ul>`;
  }}
  function draw() {{
    ctx.clearRect(0,0,IW,IH); ctx.drawImage(bg,0,0,IW,IH);
    ...
    PINS.forEach((pin, idx) => {{
      ...
      if (active) {{
        ...
        ctx.fillStyle = color + '22'; ctx.fillRect(bbox.xs, bbox.ys, bbox.xe - bbox.xs, bbox.ye - bbox.ys);
        ctx.restore();
      }}
      ctx.beginPath(); ctx.arc(pin.cx, pin.cy, active ? 11 : 9, 0, Math.PI * 2);
      ctx.fillStyle = color; ctx.fill(); ctx.lineWidth = 2; ctx.strokeStyle = '#081018'; ctx.stroke();
  ...
  function findPin(evt) {{
    const rect = canvas.getBoundingClientRect(); const sx = IW / rect.width; const sy = IH / rect.height;
    const x = (evt.clientX - rect.left) * sx; const y = (evt.clientY - rect.top) * sy;
    let nearest = -1; let minDist = 18;
    PINS.forEach((pin, idx) => {{ const dist = Math.hypot(pin.cx - x, pin.cy - y); if (dist < minDist) {{ minDist = dist; nearest = idx; }} }});
    return nearest;
  }}
  canvas.addEventListener('mousemove', (evt) => {{ hoverIndex = findPin(evt); draw(); }});
  canvas.addEventListener('mouseleave', () => {{ hoverIndex = -1; draw(); }});
  canvas.addEventListener('click', (evt) => {{ selectedIndex = findPin(evt); renderDetail(selectedIndex); draw(); }});
  bg.onload = () => {{ draw(); renderDetail(selectedIndex); }};
...
def gerar_pdf(
    img_orig: np.ndarray, img_mapa: np.ndarray,
    analises: dict, rec_texto, meta: Optional[dict] = None
) -> Optional[io.BytesIO]:
    try:
        ...
        titulo_style = ParagraphStyle("Titulo", parent=styles["Title"], textColor=VERDE, fontSize=19, alignment=TA_CENTER, spaceAfter=4)
        subtitulo_style = ParagraphStyle("Subtitulo", parent=styles["Normal"], textColor=CINZA, fontSize=9, alignment=TA_CENTER, spaceAfter=8)
        secao_style = ParagraphStyle("Secao", parent=styles["Heading2"], textColor=VERDE, fontSize=12, spaceBefore=8, spaceAfter=4)
        corpo_style = ParagraphStyle("Corpo", parent=styles["Normal"], fontSize=8, leading=11, spaceAfter=3)
        opcao_titulo_style = ParagraphStyle("OpcaoTitulo", parent=styles["Normal"], fontSize=9, leading=12, textColor=VERDE_ESCURO, fontName="Helvetica-Bold")
        opcao_body_style = ParagraphStyle("OpcaoBody", parent=styles["Normal"], fontSize=8, leading=11, leftIndent=8)
        ...
        story.append(Paragraph(f"Gerado em {time.strftime('%d/%m/%Y às %H:%M')} • Usuário: {safe(usuario)}", subtitulo_style))
        ...
        info_table = Table([
            ...
            ["Código IBGE", safe(ibge.get("id", "N/D")), "Extensão territorial",
             safe(f"{formatar_numero_br(ibge.get('area_km2'), 2)} km²" if ibge.get("area_km2") is not None else "N/D")],
            ["Densidade pop.", safe(f"{formatar_numero_br(ibge.get('densidade_hab_km2'), 2)} hab/km²" if ibge.get("densidade_hab_km2") is not None else "N/D"),
             "Microrregião", safe(ibge.get("microrregiao", "N/D"))],
        ...
        if clima and not clima.get("erro"):
            ...
            story.append(Paragraph(
                ...
                f"Precipitação: {safe(clima.get('precipitacao', 'N/D'))} mm", corpo_style))
        ...
        if alertas_inteligentes:
            ...
            for alerta in alertas_inteligentes[:6]:
                ...
                story.append(Paragraph(
                    f"<b>{safe(alerta['titulo'])}</b> ({safe(alerta['nivel'])}) — {safe(alerta['janela'])}<br/>{safe(alerta['descricao'])}<br/><b>Ação:</b> {safe(alerta['acao'])}",
        ...
        if plano_acao:
            ...
            for etapa in plano_acao[:6]:
                story.append(Paragraph(
                    ...
                    f"<b>Impacto esperado:</b> {safe(etapa['impacto'])}", corpo_style))
        ...
        img_w = 85*mm
        img_h = 55*mm
        img_table = Table([[RLImage(ob, width=img_w, height=img_h), RLImage(mb, width=img_w, height=img_h)]], colWidths=[img_w+5*mm, img_w+5*mm])
        img_table.setStyle(TableStyle([("ALIGN", (0,0), (-1,-1), "CENTER"), ("VALIGN", (0,0), (-1,-1), "MIDDLE"), ("TOPPADDING", (0,0), (-1,-1), 0), ("BOTTOMPADDING", (0,0), (-1,-1), 0)]))
        ...
        if chart_values:
            ...
            chart.x = 35; chart.y = 30; chart.height = 110; chart.width = 360
            chart.data = [chart_values]
            chart.categoryAxis.categoryNames = chart_labels
            chart.categoryAxis.labels.angle = 25; chart.categoryAxis.labels.boxAnchor = "ne"
        ...
        if isinstance(rec_texto, str):
            ...
            for nome, d in analises.items():
                ...
                story.append(Paragraph(
                    f"{safe(nome)} — {safe(d['tipo'])} | Score {d['score']}/100 | Irrigação {d['irrigacao_mm']} mm/dia",
                    secao_style))
                for chave_op in ("opcao_1", "opcao_2", "opcao_3"):
                    ...
                    for rotulo, chave in [("Culturas", "culturas"), ("Plantio", "plantio"), ("Irrigação", "irrigacao"), ("Ação", "acao")]:
        ...
        return None
        # FIX: removido o segundo 'return None' duplicado que existia aqui (dead code / erro de edição)


# ═══════════════════════════════════════════════════════════════
# ABA: REGISTRO DE PROPRIEDADE
# ═══════════════════════════════════════════════════════════════

def render_tab_registro(current_user: dict, propriedades: list):
    """
    Exibe o formulário de cadastro de propriedade e a lista de propriedades cadastradas.
    Separado em seção própria (aba fixa do topo).
    """
    ...
    with st.form("property_form_main", clear_on_submit=True):
        c1, c2 = st.columns(2)
        with c1:
            nome_prop = st.text_input("Nome da propriedade *", placeholder="Ex.: Fazenda Santa Fé")
            cidade_prop = st.text_input("Cidade / Região", placeholder="Ex.: Unaí / MG")
        ...
        with c2:
            lat_prop = st.number_input("Latitude", value=-15.7801, format="%.6f",
                                       help="Use Google Maps para obter as coordenadas da propriedade.")
            lon_prop = st.number_input("Longitude", value=-47.9292, format="%.6f")
            obs_prop = st.text_area("Observações", placeholder="Informações adicionais sobre o terreno...")
        salvar_prop = st.form_submit_button("💾 Salvar propriedade", use_container_width=True, type="primary")
    ...
    st.divider()
    typewriter("📋 Propriedades cadastradas", tag="h3")
    if propriedades:
        for prop in propriedades:
            with st.expander(f"🏡 {prop['nome']} — {prop['cidade'] or 'Sem região'} ({prop['area_ha']} ha)", expanded=False):
                col1, col2, col3 = st.columns(3)
                col1.metric("Área", f"{prop['area_ha']} ha")
                col2.metric("Latitude", f"{prop['latitude']:.4f}" if prop['latitude'] is not None else "N/D")
                col3.metric("Longitude", f"{prop['longitude']:.4f}" if prop['longitude'] is not None else "N/D")
                if prop.get("observacoes"):
                    st.caption(f"Observações: {prop['observacoes']}")
                if prop.get("latitude") is not None and prop.get("longitude") is not None:
                    mapa_local = pd.DataFrame({
                        "lat": [float(prop["latitude"])], "lon": [float(prop["longitude"])],
                        "label": [prop["nome"]],
                        "info": [f"{prop['cidade'] or 'Localização cadastrada'}<br/>Área: {prop['area_ha']} ha"],
                        "radius": [180], "color": [[34, 166, 34, 190]],
                    })
                    render_interactive_geo_map(mapa_local, center_lat=float(prop["latitude"]), center_lon=float(prop["longitude"]), zoom=12.6)
    else:
        render_priority_notice("Nenhuma propriedade cadastrada",
                               "Preencha o formulário acima para registrar sua primeira fazenda.", level="info")
...
def render_tab_analise(current_user: dict, propriedades: list, usar_ia: bool, api_key: str):
    ...
    if propriedade_ativa:
        ...
        with d2:
            ...
            if ibge_info and not ibge_info.get("erro"):
                ...
                i1, i2 = st.columns(2)
                i1.metric("🗺️ Extensão territorial",
        ...
        if st.button("Iniciar análise de grids", type="primary", use_container_width=True):
            ...
            for idx, (nome, (xs_, xe_, ys_, ye_)) in enumerate(coords.items(), start=1):
                ...
                if idx == 1 or idx == total_grids or idx % max(1, total_grids // 6) == 0:
                    ...
                    render_analysis_progress(stage_box, progress_bar, pct, "Varredura do terreno",
                                             f"{idx}/{total_grids} grids processados.")

            score_medio = round(sum(d["score"] for d in analises.values()) / len(analises), 1)
            metricas_globais.update({"score_medio": score_medio, "total_grids": len(analises)})
            ...
            localized_risks = identificar_riscos_localizados(analises, coords, clima_atual, rows, cols, area_total_ha)
            alertas_inteligentes = gerar_alertas_inteligentes(metricas_globais, riscos, clima_atual, localized_risks)
            plano_acao = montar_plano_acao_prioritario(analises, localized_risks, clima_atual, propriedade_ativa)
            risk_geo_points = estimar_coordenadas_talhoes_risco(localized_risks, propriedade_ativa)

            render_analysis_progress(stage_box, progress_bar, 88, "Preparando painel", "Organizando mapa, histórico e recomendações.")
            analysis_id = None
            if propriedade_ativa:
                analysis_id = salvar_analise_historico(
                    current_user["id"], propriedade_ativa["id"], origem_imagem,
                    img, mapa, resultado_geral, metricas_globais, analises,
                    clima_atual, riscos, coords, localized_risks,
                )
            ...
            st.session_state.update({
                ...
                "localized_risks": localized_risks, "metricas_globais": metricas_globais,
                "clima_atual": clima_atual, "origem_imagem": origem_imagem,
                "analysis_id": analysis_id, "resumo_executivo": resumo_executivo,
                "propriedade_ativa": propriedade_ativa, "resultado_geral": resultado_geral,
                "alertas_inteligentes": alertas_inteligentes, "plano_acao": plano_acao,
            ...
            if analysis_id:
                ...
                st.info("Análise avulsa gerada com sucesso. Selecione uma propriedade para salvar histórico.")
    ...
    if "analises" in st.session_state:
        ...
        if alertas_inteligentes:
            ...
            render_priority_notice("Monitoramento preventivo",
                                   "Sem alertas críticos no momento; mantenha apenas o acompanhamento preventivo.", level="low")
        ...
        for nome, d in list(analises.items())[:16]:
            ...
            barras.append(
                ...
                f"<div class='agroscan-score-track'><div style='width:{d['score']}%;background:{cor};border-radius:999px;height:14px'></div></div>"
                f"<span style='font-size:12px;min-width:140px'>{d['tipo']} {d['score']}/100</span></div>"
            )
        ...
        if riscos:
            ...
            st.success("Nenhum risco crítico identificado na análise automática atual.")
        ...
        if riscos_localizados:
            filtro_niveis = st.multiselect("Filtrar por nível de risco", ["Alto", "Médio", "Baixo"],
                                           default=["Alto", "Médio", "Baixo"], key="risk_level_filter")
            riscos_filtrados = [item for item in riscos_localizados if not filtro_niveis or item.get("nivel") in filtro_niveis]
            ...
            for item in riscos_filtrados:
                ...
                pontos_tabela.append({
                    "Ponto": len(pontos_tabela) + 1, "Grid": grid_nome,
                    "Localização": item["localizacao"],
                    "Área estimada": f"{item['area_ha']} ha" if item.get("area_ha") is not None else f"{item.get('area_pct', 0)}%",
                    "Risco": item["titulo"], "Nível": item["nivel"], "Gatilho climático": item["gatilho_clima"],
                })
                if len(pontos_tabela) >= 12:
                    break
            st.dataframe(pd.DataFrame(pontos_tabela), use_container_width=True, hide_index=True)

            if risk_geo_points:
                st.markdown("#### 🗺️ Talhões críticos no mapa estimado da fazenda")
                st.caption("Posições estimadas a partir do centro da propriedade e da malha de grids.")
                geo_df = pd.DataFrame([
                    {"lat": item["lat"], "lon": item["lon"], "Grid": item["grid"],
                     "Nível": item["nivel"], "Risco": item["risco"], "Setor": item["localizacao"]}
                ...
                geo_df["color"] = geo_df["Nível"].map({
                    "Alto": [255, 92, 92, 210], "Médio": [255, 176, 32, 205], "Baixo": [57, 217, 138, 195],
                })
                render_interactive_geo_map(geo_df, center_lat=float(geo_df["lat"].mean()), center_lon=float(geo_df["lon"].mean()), zoom=13.2)
                st.dataframe(geo_df[["Grid", "Nível", "Risco", "Setor"]], use_container_width=True, hide_index=True)
        else:
            st.info("Nenhum ponto crítico precisou ser marcado nesta leitura.")
        ...
        with st.form("feedback_form", clear_on_submit=True):
            ...
            with fb2:
                rotulo_feedback = st.selectbox("Classificação correta observada em campo",
                                               list(LABEL_PT.keys()), format_func=lambda chave: LABEL_PT.get(chave, chave))
            risco_feedback = st.selectbox("Status do risco em campo", ["Confirmado", "Parcial", "Falso positivo", "Sem risco"])
        ...
        if enviar_feedback:
            ok, msg = registrar_feedback_usuario(
                ...
                grid_feedback, analises.get(grid_feedback, {}),
                rotulo_feedback, risco_feedback, obs_feedback,
            )
            if ok:
                st.success(msg)
                st.rerun()
        ...
        if usar_ia and api_key:
            ...
            for nome, d in analises.items():
                rec = recomendacao_local(d)
                with st.expander(f"{nome} — {d['tipo']} | Score: {d['score']}/100 | Irrigação: {d['irrigacao_mm']} mm/dia", expanded=False):
                    opcoes_html = []
                    for chave_op in ("opcao_1", "opcao_2", "opcao_3"):
                        op = rec[chave_op]
                        num = chave_op.split("_")[1]
                        opcoes_html.append(
                            ...
                            f"⚙️ <strong>Ação:</strong> {op['acao']}</div>"
                        )
    ...
    if propriedade_ativa:
        ...
        if historico_prop:
            hist_df = pd.DataFrame([
                {"ID": item["id"], "Data": item["created_at"].replace("T", " ")[:16],
                 "Fonte": item["image_source"], "Resultado": item["resultado_geral"],
                 "NDVI": round(float(item["ndvi_global"]), 3), "Score": round(float(item["score_medio"]), 1)}
            ...
            if len(historico_prop) >= 2:
                ...
                if registro_a["id"] != registro_b["id"]:
                    ...
                    if analise_a and analise_b:
                        ...
                        if img_a_bgr is not None and img_b_bgr is not None:
                            ...
                            with tab_cmp_img:
                                slider_comparacao(img_a_bgr, img_b_bgr, 50,
                            ...
                            with tab_cmp_ndvi:
                                if mapa_a_bgr is not None and mapa_b_bgr is not None:
                                    slider_comparacao(mapa_a_bgr, mapa_b_bgr, 50,
                                    ...
                                    render_priority_notice("Mapas NDVI indisponíveis",
                                                           "Algumas análises antigas não possuem mapa salvo para esta comparação.", level="info")
                        else:
                ...
                render_priority_notice("Histórico insuficiente",
                                       "Salve pelo menos 2 análises desta propriedade para liberar a comparação temporal.", level="info")
        else:
            render_priority_notice("Sem histórico salvo",
                                   "Ainda não há análises salvas para esta propriedade.", level="info")
    else:
...
# Título principal (visível apenas fora da topbar)
st.markdown(
...
# ── Navbar interna via tabs do Streamlit (sem recarregar o login)
# A seleção de aba é mantida na mesma sessão e evita retorno à tela de login.
# render_topbar(active_tab)

# ── Sidebar (configurações e IA) ─────────────────────────────
with st.sidebar:
    st.header("Configurações")
    st.success(f"Usuário: {current_user['username']}")
